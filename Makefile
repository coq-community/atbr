...
install-natdynlink:
	cd "." && for i in $(CMXSFILES); do \
	 ...
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/ATBR/$$i; \
	done
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/ATBR && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "ATBR" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
